...
{
	Serial.println("Initializing in mode 1: standard Joank fix as tested in ES3. Joan thinks it has a bug but it seems to do well enough.");
	//uint8_t msg_len = 180;
    if (!RHSPIDriver::init())
	return false;
    ...
    uint8_t pkt_field1[] = { 0x00, msg_len, RH_RF24_SYNC_CONFIG_MANCH, 0x0 };
...
void RH_RF24::setPreambleLength(uint16_t bytes)
{
    ...
    uint8_t config2[] = { (uint8_t)14  };
    set_properties(RH_RF24_PROPERTY_PREAMBLE_CONFIG_STD_1, config2, sizeof(config2));
}
